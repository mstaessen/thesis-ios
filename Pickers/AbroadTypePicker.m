...
@interface AbroadTypePicker ()
@property (nonatomic, retain, readwrite) NSDictionary *types;
@end

@implementation AbroadTypePicker

@synthesize abroadTypePickerDelegate = _abroadTypePickerDelegate;
...
- (id)init:(id<AbroadTypePickerDelegate>)delegateObject
{
    ...
    if (self) {
        // Initialization code
		self.delegate = self;
		self.abroadTypePickerDelegate = delegateObject;
		self.showsSelectionIndicator = YES;
		
		_types = [[NSDictionary alloc] initWithObjectsAndKeys:
				  @"Hotel", @"1", @"Restaurant", @"5", @"Other...", @"6", nil];
		...
		[[self abroadTypePickerDelegate] didSelectType:_type withName:_typeName];
    }
    return self;
}
